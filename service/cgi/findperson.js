...
exports.call = function( req, res ) {
    ...
    if( !requireDataOk(reqData) ) {
        ...
        try {
            ...
            global.airaFaceLitePersonDb.find( reqData.uuid, function(personList) {
                //if( personList.length > 0 ) {
                    res.status( 200 ).json( { 
                        ...
                        person_list : personList.slice( shift, shift + sliceLength ).filter( item => {
                            delete item["___id"];
                            delete item["___s"]; 
                ...
                //}
                //else res.status( 200 ).json( { message : "ok", total_length : 0, slice_shift : shift, slice_length : sliceLength, person_list : [] } );
            } );
        }
        catch(e) {
