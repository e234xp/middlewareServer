...
module.exports = () => {
  ...
  function readRegisterPhoto(uuid) {
    ...
    try {
      ...
      global.spiderman.systemlog.writeError(`readRegisterPhoto ${registerPhotoFile} ${e}`);
  ...
  function trigger({ action, data }) {
    global.spiderman.systemlog.generateLog(4, `domain trigger-email-command trigger ${action}`);

    const triggeredKey = generateTriggeredKey({ data });
    if (triggered[triggeredKey]) return;

    const {
      host, port, security, user: email, pass: password, from: sender, subject, data_list: fields, custom_data: note, language, to: toEmails, cc: ccEmails, bcc: bccEmails,
    } = action;
    ...
    global.spiderman.systemlog.generateLog(4, `domain trigger-email-command trigger ${result}`);
  ...
  function generateMessage({
    ...
    global.spiderman.systemlog.generateLog(4, `domain trigger-email-command generateMessage ${fields}`);
    ...
    Object.keys(fields).forEach((key1) => {
      switch (key1) {
        ...
        case 'verified_timestamp':
          message += (`${transform.Timestamp || 'Timestamp'}: ${data[key1] || ''}\n`);
          break;
        case 'verified_datetime':
          message += (`${transform.Datetime || 'Datetime'}: ${global.spiderman.dayjs(data[key1]).format('YYYY/MM/DD HH:mm:ss') || ''}\n`);
          break;
        case 'source_id':
          const { name } = global.domain.device.findByUuid(data[key1]);
          message += (`${transform.SourceDevice || 'SourceDevice'}: ${name || ''}\n`);
          break;
