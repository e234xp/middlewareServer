...
module.exports = () => {
  function useLimitCgiNumber({ routesNeedToLimit = [] }) {
    return (req, res, next) => {
      ...
      const { cgiCounter, maxCgiNumber } = global.spiderman.param;
      if (cgiCounter + 1 > maxCgiNumber) {
        res.status(429).json({ message: 'Too Many Requests, server allows upto max 50 request concurrently.' });
        return;
      }

      global.spiderman.param.cgiCounter += 1;

      req.on('close', () => {
        if (global.spiderman.param.cgiCounter <= 0) {
          global.spiderman.param.cgiCounter = 0;
          return;
        }
